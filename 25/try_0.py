...
train_x, val_x, train_y, val_y = train_test_split(train_x, train_y, test_size=0.25, random_state=4096)
val = [(val_x, val_y)]

# stage 1
model = xgb.XGBRegressor(learning_rate=0.1126, max_depth=6, subsample=0.5, n_jobs=32, tree_method='hist', n_estimators=1126)
model.fit(train_x, train_y, eval_set=val, eval_metric='mae', early_stopping_rounds=6, xgb_model='../23/m0.model')
...
# stage 2
model = xgb.XGBRegressor(learning_rate=0.01126, max_depth=4, subsample=0.75, n_jobs=32, tree_method='hist', n_estimators=1126)
model.fit(train_x, train_y, eval_set=val, eval_metric='mae', early_stopping_rounds=10, xgb_model='m0.model')
model.save_model('m0.model')
with open('status.txt', 'w') as f:
