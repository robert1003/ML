...
def run(models=None,yid=None,gen=True,track=1,validation=None,cross_val=False,add_const=False):
    ...
    readdata()
    global x,y,y_res,x_test,y_test,x_val,y_val,x_intest,y_intest_res,y_intest,ind,score1,score2,test_score1,test_score2
            ...
            if validation is None:
                ...
                if (track==1 and s1<mn1) or (track==2 and s2<mn2):
                    ...
                    if gen:
                        ...
                        y_intest_res[i]=model.predict(x_intest).copy()
            else:
                if cross_val:
                    s1/=len(x)*(len(ind)-1)
                    s2/=len(x)*(len(ind)-1)
