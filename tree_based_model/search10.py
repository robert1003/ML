...
for per in feature_percent:
    X_train, X_test, y_train, y_test = train_test_split(X[:, feature_importances[:, 0] > per], y, test_size=0.2, random_state=1126)
    for v in itertools.product(*values):
        ...
        Ein.append(300 * mean_absolute_error(y_train, model.predict(X_train)))
        Eval.append(300.* mean_absolute_error(y_test, model.predict(X_test)))
